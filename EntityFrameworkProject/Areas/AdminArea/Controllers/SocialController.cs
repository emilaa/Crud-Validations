...
namespace EntityFrameworkProject.Areas.AdminArea.Controllers
{
    ...
    public class SocialController : Controller
    {
        ...
        {
            IEnumerable<Social> socials = await _context.Socials.Where(m => !m.IsDeleted)
                .AsNoTracking()
        ...
        public async Task<IActionResult> Create(Social social)
        {
            try
            {
                ...
                bool isExist = await _context.Socials
                    .AnyAsync(m => m.Name.Trim() == social.Name.Trim() && m.Url.Trim() == social.Url.Trim());
        ...
        public async Task<IActionResult> Update(int id, Social social)
        {
            try
            {
                ...
                if (dbSocial.Name.ToLower().Trim() == social.Name.ToLower().Trim()
                    && dbSocial.Url.ToLower().Trim() == social.Url.ToLower().Trim())
                {
                    return RedirectToAction(nameof(Index));
                }
